...
=======
def _sentiment_job(usb_path: str) -> None:
main
    ...
    news = fetch_news(
        am=(now_et.hour < 12),
        max_items=SENTIMENT_MAX_ITEMS,
        google_key=vals.get("GOOGLE_API_KEY"),
        cse_id=vals.get("GOOGLE_CSE_ID"),
    )
    items = [
...
=======
    ...
    if ok:
        ...
        values = dotenv_values(os.path.join(usb_path, "keys.env")) or {}

    news = fetch_news(
        am=now_et.hour < 12,
        max_items=SENTIMENT_MAX_ITEMS,
        google_key=values.get("GOOGLE_API_KEY"),
        cse_id=values.get("GOOGLE_CSE_ID"),
    )
    ...
    scored = score_articles_openai(items, api_key=values.get("OPENAI_API_KEY"))
    blob = to_json_blob(scored)
    ...
    with open(path, "w", encoding="utf-8") as handle:
        json.dump(blob, handle, ensure_ascii=False, indent=2)
...
=======
def _prune_old(folder, keep_days: int) -> None:
main
