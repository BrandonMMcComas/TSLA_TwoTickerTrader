...
def _dedupe_by_url(items: List[NewsItem], cap: int) -> List[NewsItem]:
    ...
    return out
def _cse_query(q: str, key: str, cse_id: str, date_restrict: str = "d1") -> List[NewsItem]:
    endpoint = "https://www.googleapis.com/customsearch/v1"
    params = {"key":key,"cx":cse_id,"q":q,"dateRestrict":date_restrict,"num":10,"sort":"date"}
    r = requests.get(endpoint, params=params, timeout=15); r.raise_for_status()
    data = r.json(); items: List[NewsItem] = []
...
def _rss_query(q: str) -> List[NewsItem]:
    ...
    feed = feedparser.parse(url); out: List[NewsItem] = []
...
def fetch_news(am: bool, max_items: int, google_key: str | None, cse_id: str | None) -> List[NewsItem]:
    ...
    items: List[NewsItem] = []; use_cse = bool(google_key and cse_id)
    for q in queries:
        try:
            items.extend(_cse_query(q, google_key, cse_id, "d1") if use_cse else _rss_query(q))
        except Exception:
            pass
