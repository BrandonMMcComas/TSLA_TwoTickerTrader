...
class TradeControl(QWidget):
    def __init__(self, config: AppConfig) -> None:
        ...
        row = QHBoxLayout(); row.addWidget(self.btn_start); row.addWidget(self.btn_stop); row.addStretch(1)
        v.addLayout(row)
        ...
        v.addWidget(self.lbl_acct); v.addWidget(self.lbl_hold); v.addWidget(self.lbl_preview)
    ...
    def _start(self):
        self._connect()
        if not self._alp:
            self.lbl_acct.setText("Account: (no Alpaca keys)"); return
    ...
    def _tick(self):
        ...
        if self._alp:
            try:
                ...
                self.lbl_acct.setText(f"Account: {classification} | Equity≈{eq:,.0f} | Settled≈{nmbp:,.0f}")
                # holdings snapshot (best-effort)
                try:
                    posL = self._alp.get_position("TSLL"); posD = self._alp.get_position("TSDD")
                except Exception:
                    posL = posD = None
                if posL:
                    self.lbl_hold.setText(f"Holding: TSLL qty={getattr(posL,'qty',0)} avg={getattr(posL,'avg_entry_price','?')}")
                elif posD:
                    self.lbl_hold.setText(f"Holding: TSDD qty={getattr(posD,'qty',0)} avg={getattr(posD,'avg_entry_price','?')}")
                else:
        ...
        # (Dashboard computes p_blend using last sentiment file; for preview, keep it simple: use p_up)
        want_tsll = (p_up == p_up) and (p_up >= state.gate_threshold)
        sym = "TSLL" if want_tsll else "TSDD"
        q = get_quote(sym)
        entry = compute_entry_limit("BUY", q["bid"], q["ask"], q["last"], state.slippage_bps)
